...
try {
  ...
  */
  ...
  await p.check('input[type=checkbox][value="Oficina Mão na Roda"]')
  ...
  */
  ...
  // ── 5f. O modo atendimento, e o projeto que acabou ────────────────────────
  {
    ...
    /*
      A Bocha Paralímpica acabou, e a associação mandou tirar do site inteiro. A rota do
      projeto precisa responder 404 — não basta o item sair da lista, porque quem tem o link
      antigo continua chegando lá.
    */
    const resposta = await a.request.get(`${BASE}/projetos/bocha-paralimpica`)
    ok('a rota do projeto encerrado responde 404', resposta.status() === 404, resposta.status())
